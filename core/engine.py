...
if PAPER_TRADING_MODE:
    api_key = API_KEYS.get('testnet_futures', {}).get('key', '')
    api_secret = API_KEYS.get('testnet_futures', {}).get('secret', '')
    testnet_flag = True
else:
...
async def panic_sell_position(symbol, client_instance=None):
    ...
    if not cli:
        try:
            cli = await BinanceAsyncClient.create(api_key, api_secret, testnet=testnet_flag)
...
async def run_bot(log_callback=None, investment_amount=None, selected_symbol=None, status_callback=None):
    ...
    global client
    try:
        client = await BinanceAsyncClient.create(api_key, api_secret, testnet=testnet_flag)
